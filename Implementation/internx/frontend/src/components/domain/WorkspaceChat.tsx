...
export function WorkspaceChat({ project }: Props) {
  const { user } = useAuth();
  ...
  const businessId = typeof project.businessId === "string" ? project.businessId : project.businessId._id;
  const businessName = typeof project.businessId === "string" ? "Business" : project.businessId.name ?? "Business";

  const loadMessages = useCallback(async () => {
    try {
      const res = await messagesApi.forProject(project._id);
      setMessages(res);
      setError(null);
    } catch (err) {
  ...
  return (
    <Panel padding="p-0" className="flex flex-col min-h-[500px] max-h-[600px]">
      ...
      <div className="flex items-center gap-3 border-b border-border-default px-5 py-3">
        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand/15 text-[11px] font-semibold text-brand">
          {getInitials(businessName)}
        </div>
        <div>
          <p className="text-[13px] font-medium text-text">{businessName}</p>
          <p className="text-[11px] text-text-subtle">Project Chat · {project.title}</p>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        {messages.length === 0 && !error && (
          <EmptyState
            ...
            description="Start a conversation with the business about this project."
